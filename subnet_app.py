...
class OctetFrame(tk.Frame):
    ...
    def create_widgets(self):
        ...
        # Create labels for bit positions (1 to 8)
        for i in range(8):
    ...
    def toggle_bit(self, index):
        ...
        self.buttons[index].config(text=str(self.bits[index]))
        if self.bits[index] == 1:
            self.value_labels[index].config(fg="red", font=("Helvetica", 10, "bold"))
        else:
            self.value_labels[index].config(fg="black", font=("Helvetica", 10))
...
class SubnettingApp(tk.Tk):
    ...
    def create_widgets(self):
        ...
        octets_frame = tk.Frame(self)
        octets_frame.pack(pady=10)
